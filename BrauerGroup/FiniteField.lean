...
lemma BrauerTrivial : ∀(A : CSA K), ∃(n : ℕ)(_ : NeZero n),
    Nonempty (A ≃ₐ[K] Matrix (Fin n) (Fin n) K) := fun A ↦ by
  obtain ⟨n, hn, D, hD1, hD2, ⟨iso⟩⟩ := Wedderburn_Artin_algebra_version K A
  have : Finite D := Module.finite_iff_finite|>.1 <| is_fin_dim_of_wdb K A n D iso
  let _ := littleWedderburn D
  have eq1 := is_central_of_wdb K A n D iso|>.center_eq_bot
...
theorem trivialBrauer : ∀(a : BrauerGroup K), a = 1 := fun a ↦ by
  ...
  exact ⟨1, n, one_ne_zero, hn.1, ⟨BrauerGroup.dim_one_iso A|>.trans iso⟩⟩
