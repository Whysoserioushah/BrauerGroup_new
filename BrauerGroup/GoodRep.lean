...
lemma centralizerιRange : Subalgebra.centralizer F A.ι.range = A.ι.range := by
  ...
  { __ := A.ι.range
    mul_comm := by
      ...
      exact fun _ _ ↦ by simp [← map_mul, mul_comm]
    exists_inverse := by
      rintro _ ⟨x, rfl⟩ hx
      refine ⟨A.ι x⁻¹, ?_, ?_⟩
      · simp
...
lemma compare_toTwoCocycles' (x_ : Π σ, A.conjFactor σ) (y_ : Π σ, B.conjFactor σ) :
  ...
  fconstructor
  · refine fun σ ↦ A.pushConjFactorCoeffAsUnit B (x_ σ) (y_ σ)
  intro σ τ
...
lemma conjFactor_linearIndependent (A : GoodRep K X) (x_ : Π σ, A.conjFactor σ) :
...
def GoodRep.toH2 {X : BrauerGroup F} (A : GoodRep K X) (x_ : Π σ, A.conjFactor σ) :
    H2 (galAct F K) :=
  Quotient.mk'' <| twoCocyclesOfIsMulTwoCocycle (f := A.toTwoCocycles x_) (A.isTwoCocyles x_)
...
lemma RelativeBrGroup.toSnd_wd (X : RelativeBrGroup K F) (A : GoodRep K X.1)
    (x_ : Π σ, A.conjFactor σ) : toSnd X = A.toH2 x_ := by
