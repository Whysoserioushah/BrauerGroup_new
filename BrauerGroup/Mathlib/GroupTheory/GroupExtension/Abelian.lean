...
theorem isConj_iff_sub_mem_oneCoboundaries (s₁ s₂ : (toGroupExtension φ).Splitting) :
    ...
    (splittingToOneCocycle φ s₁ - splittingToOneCocycle φ s₂).1 ∈
    groupCohomology.oneCoboundaries (toRep φ) := by
  erw [sub_mem_comm_iff]
...
theorem sub_mem_twoCoboundaries_of_toOfMulDistribMulAction_equiv
    ...
    (S.toTwoCocycle - S'.toTwoCocycle).1 ∈
...
theorem exists_cochain_of_sub_mem_twoCoboundaries
    ...
    (h : (f - f').1 ∈ groupCohomology.twoCoboundaries (Rep.ofMulDistribMulAction G N)) :
    ∃ x : G → N, ∀ g₁ g₂ : G, Additive.toMul (α := N) (f (g₁, g₂)) *
    (Additive.toMul (α := N) (f' (g₁, g₂)))⁻¹ = g₁ • x g₂ * (x (g₁ * g₂))⁻¹ * x g₁ := by
  simp only [groupCohomology.twoCoboundaries, groupCohomology.dOne,
    Rep.ofMulDistribMulAction_ρ_apply_apply, groupCohomology.twoCocycles.val_eq_coe,
    LinearMap.mem_range, LinearMap.coe_mk, AddHom.coe_mk] at h
  obtain ⟨x, hx⟩ := h
  ...
  calc
    ...
    _ = _ := by
      have := congr($hx ⟨g₁, g₂⟩)
      simp at this
      rw [← this, toMul_add, toMul_sub]
      erw [Rep.ofMulDistribMulAction_ρ_apply_apply, toMul_ofMul, div_eq_mul_inv]
      rfl
...
noncomputable def equivH2 :
  ...
  toFun := Quotient.lift toH2 fun x1 x2 ⟨equiv⟩ ↦ (Quotient.eq (r := Submodule.quotientRel _)).mpr <|
    (Submodule.quotientRel_def _).mpr <| by
      convert sub_mem_twoCoboundaries_of_equiv equiv x1.extension.surjInvRightHom
        x2.extension.surjInvRightHom using 1
      apply exists_congr
      exact fun _ ↦ Subtype.ext_iff
  invFun := Quotient.lift ofH2 fun _ _ h ↦ (Quotient.eq (r := setoid N G)).mpr
    ⟨OfMulDistribMulActionWithSection.toOfMulDistribMulActionEquivOfCochain
      (exists_cochain_of_sub_mem_twoCoboundaries <|
        (Submodule.quotientRel_def _).mp (by
          change (LinearMap.range (groupCohomology.shortComplexH2
            (Rep.ofMulDistribMulAction G N)).moduleCatToCycles).quotientRel _ _ at h
          rw [Submodule.quotientRel_def] at h ⊢
          -- apply Subtype.ext_iff at h
          obtain ⟨f, hf⟩ := h
          use f
  ...
  right_inv := by
    ...
    unfold ofH2 toH2
    rw [← Quotient.mk]
    have := OfMulDistribMulActionWithSection.sub_mem_twoCoboundaries_of_toOfMulDistribMulAction_equiv
      (Equiv.refl
    ...
    simp only [groupCohomology.twoCocycles.val_eq_coe] at this
    simp only [Quotient.lift_mk, Quotient.eq (r := Submodule.quotientRel _),
    ...
    convert hg using 1
    congr!
    exact GroupExtension.OfMulDistribMulActionWithSection.toTwoCocycle_ofTwoCocycle f |>.symm
