...
noncomputable def toTwoCocycle : groupCohomology.twoCocycles (Rep.ofMulDistribMulAction G N) where
...
theorem mul_def (e₁ e₂ : TwistedProduct f) :
    e₁ * e₂ = ⟨e₁.left * e₁.right • e₂.left * Additive.toMul (α := N) (f (e₁.right, e₂.right)),
    e₁.right * e₂.right⟩ := rfl

@[simp]
theorem mul_left (e₁ e₂ : TwistedProduct f) : (e₁ * e₂).left =
  e₁.left * e₁.right • e₂.left * Additive.toMul (α := N) (f (e₁.right, e₂.right)) := rfl

@[simp]
theorem mul_right (e₁ e₂ : TwistedProduct f) : (e₁ * e₂).right = e₁.right * e₂.right := rfl

instance : One (TwistedProduct f) where
  one := ⟨(Additive.toMul <| f (1, 1))⁻¹, 1⟩

theorem one_def : (1 : TwistedProduct f) = ⟨(Additive.toMul <| f (1, 1))⁻¹, 1⟩ := rfl

@[simp]
theorem one_left : (1 : TwistedProduct f).left = (Additive.toMul <| f (1, 1) : N)⁻¹ := rfl

@[simp]
theorem one_right : (1 : TwistedProduct f).right = 1 := rfl

instance : Inv (TwistedProduct f) where
  inv e := ⟨(e.right⁻¹ • e.left * Additive.toMul (α := N) (f (e.right⁻¹, e.right)) *
    (Additive.toMul (α := N) (f (1, 1))))⁻¹, e.right⁻¹⟩

theorem inv_def (e : TwistedProduct f) :
    e⁻¹ = ⟨(e.right⁻¹ • e.left * Additive.toMul (α := N) (f (e.right⁻¹, e.right)) *
    (Additive.toMul (α := N) (f (1, 1))))⁻¹, e.right⁻¹⟩ := rfl

@[simp]
theorem inv_left (e : TwistedProduct f) :
    e⁻¹.left = (e.right⁻¹ • e.left * Additive.toMul (α := N) (f (e.right⁻¹, e.right)) *
    (Additive.toMul (α := N) (f (1, 1))))⁻¹ := rfl

@[simp]
theorem inv_right (e : TwistedProduct f) : e⁻¹.right = e.right⁻¹ := rfl

/-- The group structure associated to an extension corresponding to a 2-cocycle -/
instance : Group (TwistedProduct f) where
  mul_assoc := by
    intro ⟨n₁, g₁⟩ ⟨n₂, g₂⟩ ⟨n₃, g₃⟩
    ext
...
def inl : N →* TwistedProduct f where
...
theorem left_inl (n : N) :
    (inl n : TwistedProduct f).left = n * (Additive.toMul (α := N) (f (1, 1)))⁻¹ := rfl

@[simp]
theorem right_inl (n : N) : (inl n : TwistedProduct f).right = 1 := rfl

theorem mem_range_inl (e : TwistedProduct f) : e ∈ inl.range ↔ e.right = 1 :=
  ⟨fun ⟨n, hn⟩ ↦ hn ▸ right_inl n, fun h ↦ ⟨e.left * Additive.toMul (α := N) (f (1, 1)),
    TwistedProduct.ext (by rw [left_inl, mul_inv_cancel_right]) (by rw [right_inl, h])⟩⟩
...
def rightHom : TwistedProduct f →* G where
  toFun := right
  map_one' := rfl
  map_mul' _ _ := rfl

@[simp]
theorem rightHom_eq_right : (rightHom : TwistedProduct f → G) = right := rfl
...
theorem inl_smul_eq_conj_inl (g : G) (n n' : N) :
    (inl (g • n) : TwistedProduct f) = ⟨n', g⟩ * inl n * ⟨n', g⟩⁻¹ := by
...
def extensionOfTwoCocycle : GroupExtension N (TwistedProduct f) G where
  inl := TwistedProduct.inl
  rightHom := TwistedProduct.rightHom
  inl_injective := TwistedProduct.inl_injective
  range_inl_eq_ker_rightHom := TwistedProduct.range_inl_eq_ker_rightHom
  rightHom_surjective := TwistedProduct.rightHom_surjective

theorem extensionOfTwoCocycle_inl :
    (extensionOfTwoCocycle f).inl = TwistedProduct.inl := rfl

theorem extensionOfTwoCocycle_rightHom :
    (extensionOfTwoCocycle f).rightHom = TwistedProduct.rightHom := rfl
...
def sectionOfTwoCocycle : (extensionOfTwoCocycle f).Section where
  ...
  rightInverse_rightHom := by
      intro _
      simp only [extensionOfTwoCocycle_rightHom, TwistedProduct.rightHom_eq_right]
...
def ofTwoCocycle : OfMulDistribMulActionWithSection N G where
  ...
  smul_eq_conjAct := by
    ...
    rw [inl_conjAct_comm, extensionOfTwoCocycle_inl, ← TwistedProduct.inl_smul_eq_conj_inl,
      Section.coe_mk]
...
theorem toTwoCocycle_ofTwoCocycle :
  ...
  simp only [sectionOfTwoCocycle, TwistedProduct.mul_def, TwistedProduct.inv_left,
    TwistedProduct.inv_right, mul_inv, smul_mul', smul_inv', Section.coe_mk, smul_one, mul_one,
    one_mul, inv_one, mul_inv_cancel, TwistedProduct.smul_map_inv_eq, inv_inv,
    TwistedProduct.map_one_snd (g₁ * g₂)]
  rw [mul_right_comm _ _ ((g₁ * g₂) • Additive.toMul (α := N) (f (1, 1)))⁻¹,
    mul_inv_cancel_right, mul_assoc, inv_mul_cancel_right]
  ext <;> simp only [TwistedProduct.left_inl, TwistedProduct.right_inl]
...
noncomputable def ofTwoCocycleToTwoCocycleEquiv (S : OfMulDistribMulActionWithSection N G) :
  ...
  inl_comm := by
    ...
    ext n
    simp only [MulEquiv.coe_mk, Equiv.coe_fn_mk, Function.comp_apply, TwistedProduct.left_inl,
      map_mul, map_inv, inl_toTwoCocycle, mul_one, mul_inv_cancel_right,
      TwistedProduct.right_inl, inv_mul_cancel_right]
  rightHom_comm := by
    unfold ofTwoCocycle extensionOfTwoCocycle
    ext ⟨n, g⟩
    simp only [MulEquiv.coe_mk, Equiv.coe_fn_mk, Function.comp_apply, map_mul, rightHom_inl,
      Section.rightHom_section, one_mul, TwistedProduct.rightHom_eq_right]
  section_comm := by
...
noncomputable def toOfMulDistribMulActionEquivOfCochain
  ...
  map_mul' := by
    intro ⟨n₁, g₁⟩ ⟨n₂, g₂⟩
    simp only [ofTwoCocycle, TwistedProduct.mul_def]
    ext
  ...
  inl_comm := by
    ...
    simp only [ofTwoCocycle, extensionOfTwoCocycle, TwistedProduct.inl, MulEquiv.coe_mk,
      Equiv.coe_fn_mk, MonoidHom.coe_mk, OneHom.coe_mk, Function.comp_apply, ← h, mul_assoc,
      inv_mul_cancel_left]
  rightHom_comm := by
    ext ⟨_, g⟩
    simp only [ofTwoCocycle, extensionOfTwoCocycle_rightHom, TwistedProduct.rightHom,
