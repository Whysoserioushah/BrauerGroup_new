...
def maxSubfieldOfDiv (a : D): SubField K D where
  __ := (Algebra.adjoin K (insert a (Subalgebra.center K D)))
  mul_comm := fun x y hx hy ↦ Algebra.adjoin_induction₂ (ha := hx) (hb := hy)
    (fun α hα β hβ ↦ by
      simp only [Set.mem_insert_iff, SetLike.mem_coe] at hα hβ
    ...
    (fun x y z hxz hyz ↦ by rw [mul_add, add_mul, hxz, hyz])
    (fun x y z hxz hyz ↦ by rw [mul_assoc, hyz, ← mul_assoc, hxz, mul_assoc])
    (fun x y z hxy hxz ↦ by sorry)
  inverse := sorry

theorem maxsubfield_is_sep : ∃ a : D, Algebra.IsSeparable K (maxSubfieldOfDiv K D a)
    ∧ IsMaximalSubfield K D (maxSubfieldOfDiv K D a) := by sorry
-- We need to change the lemmas for subfields such that subfield itself is a subalgebra and
-- a field instead of doing subfield.1 everywhere
