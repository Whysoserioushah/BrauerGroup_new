...
theorem IsSplit.of_algEquiv {B : Type v} [Ring B] [Algebra K B]
...
extension `L'` of `L`. -/
theorem IsSplit.of_isScalarTower (L' : Type*) [Field L'] [Algebra K L'] [Algebra L L']
    [IsScalarTower K L L'] (h : IsSplit K A L) : IsSplit K A L' := by
  obtain ⟨n, hn, ⟨iso⟩⟩ := h
  refine ⟨n, hn, ⟨?_⟩⟩
...
theorem isSplit_iff_baseChange_eq_one :
  ...
  · rw [baseChange_mk, ← mk_self_eq_one, mk_eq_mk] at h
    ...
    obtain ⟨isoD⟩ := IsSimpleRing.wedderburn_artin_divisionring_unique L
      (Matrix (Fin m) (Fin m) L) e'.symm AlgEquiv.refl
    exact ⟨p, hp.out, ⟨wdb.trans isoD.mapMatrix⟩⟩
...
theorem isSplit_congr {B : Type u} [Ring B] [Algebra K B] [IsSimpleRing B]
    [Algebra.IsCentral K B] [FiniteDimensional K B]
    (h : BrauerGroup.mk K A = BrauerGroup.mk K B) :
    IsSplit K A L ↔ IsSplit K B L := by
  rw [isSplit_iff_baseChange_eq_one, isSplit_iff_baseChange_eq_one, h]
