...
\begin{construction}
  ...
  $B\otimes_{K}\End_{A}M$-module structure by $M^{f}$. \leanok \lean{IsMod}
\end{construction}

\begin{lemma}
  Let $f : B \to A$ be a $K$-algebra homomorphism, $M^{f}$ is finitely generated
  as a $B\otimes_{K}\End_{A}M$-module. \leanok \lean{module_inst_findim}
\end{lemma}
\begin{proof}
...
\begin{lemma}
  ...
  are isomorphic as $B\otimes_{K}\End_{A}M$-module. \leanok \lean{iso_fg}
\end{lemma}
...
\begin{theorem}[Skolem-Noether]\label{thm:skolem-noether}
  ...
  that $g = x f x^{-1}$. \leanok \lean{SkolemNoether'}
\end{theorem}
