...
% \begin{lemma}
%   There exists an $n \in \mathbb{N}$ such that $M \cong S^{n}$ as $\cross_{\mfc}$-module.
%   \leanok
%   \lean{map_mul_proof.M_directSum}
% \end{lemma}

% \begin{proof}
%   By~\cref{lem:direct-sum-simple-mod} and~\cref{lem:direct-sum-simple-mod2}, there exists an indexing set $J'$ such that $M \cong \bigoplus_{i\in J'}S$ as $\cross_{\mfc}$-modules and consequently isomorphic as $F$-vector spaces. Since $M$ is finite $F$-dimensional, $J'$ must be finite. Hence $n := |J'|$ works.
